"""FileOrganizer — Rule engine, event grouping, scheduling, rename templates."""
...
class ScheduleManager:
    ...
    def delete_task(name):
        if sys.platform != 'win32':
            return False
        try:
            subprocess.run(['schtasks', '/delete', '/tn', f'FileOrganizer\\{name}', '/f'],
    ...
    def list_tasks() -> list:
        ...
        try:
            result = subprocess.run(
                ['schtasks', '/query', '/tn', 'FileOrganizer\\', '/fo', 'CSV', '/nh'],
            ...
            for line in result.stdout.strip().split('\n'):
                if line.strip():
                    ...
                    if len(parts) >= 3:
                        tasks.append({
                            'name': parts[0].replace('FileOrganizer\\', ''),
