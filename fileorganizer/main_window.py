"""FileOrganizer — Main application window."""
...
from fileorganizer.dialogs import RelationshipGraphWidget
from fileorganizer.profiles import (
    get_active_profile, get_active_profile_name, get_profile_names,
    set_active_profile, BUILTIN_PROFILES
)
from fileorganizer.scan_mixin import ScanMixin
from fileorganizer.apply_mixin import ApplyMixin

class FileOrganizer(ScanMixin, ApplyMixin, QMainWindow):
    OP_AEP   = 0
    OP_CAT   = 1
    ...
    def __init__(self, *, background_automation: bool = False):
        ...
        self.setWindowTitle("FileOrganizer v8.5.20")
    ...
    class _NumericItem(QTableWidgetItem):
        ...
        def __lt__(self, other):
            if isinstance(other, FileOrganizer._NumericItem):
    ...
    def _export_html(self):
        ...
        if op == self.OP_FILES:
            ...
            items = self.aep_items
        if not items: self._log("No items to export"); return
        path, _ = QFileDialog.getSaveFileName(self, "Export HTML Report", "FileOrganizer_Report.html", "HTML Files (*.html)")
        if not path: return
        try:
    ...
    def _setup_tray(self):
        ...
        self._tray.setToolTip("FileOrganizer — Watch Mode")
        ...
        act_show = tray_menu.addAction("Show FileOrganizer")
    ...
    def _watch_pause(self):
        ...
        if self._watch_manager and self._watch_manager.is_active:
            ...
            if self._tray:
                self._tray.showMessage("FileOrganizer", "Watch mode paused",
        ...
        if self.btn_watch.isChecked():
            ...
            if dlg.exec():
                ...
                if folders:
                    ...
                    if self._tray:
                        self._tray.show()
                        self._tray.showMessage("FileOrganizer", f"Watching {len(folders)} folder(s)",
    ...
    def closeEvent(self, event):
        ...
        if (self._watch_manager and self._watch_manager.is_active
            ...
            if settings.get('minimize_to_tray', True):
                self.hide()
                self._tray.showMessage("FileOrganizer", "Minimized to tray — Watch mode active",
                                       QSystemTrayIcon.MessageIcon.Information, 2000)
                event.ignore()
                return
        self._save_settings()
