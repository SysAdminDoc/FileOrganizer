...
def record_api_call(provider: str, tokens_used: int, model: str = None):
    ...
    Estimates cost based on provider and token count ($ per token):
    - DeepSeek V4 flash: ~$0.55 per 1M input tokens = $0.00000055/token
    - GitHub Models: ~$0.75 per 1M tokens = $0.00000075/token
    - Ollama: $0.00 (local)
    """
    if provider == 'ollama':
        return
